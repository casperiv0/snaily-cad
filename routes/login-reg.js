                ...
                bcrypt.hash(password, saltRounds, function (err, hash) {
                    ...
                    connection1.query(q1, (err, result) => {
                        if (result.length > 0) {
                            ...
                            if (username && password) {
                                connection1.query("INSERT INTO users (`username`, `email`, `password`, `admin`, `leo`, `ems_fd`, `dispatch`, `cadID`,  `main_administrator_sM7a6mFOHI`, `orderID`, `expired`, `banned`) VALUES ('" + username + "', '" + username + "@" + cadID + "', '" + hash + "', 'no', 'no', 'no', 'no', '" + cadID + "', 'pi75PugYho', '', 'no', 'false')", function (error, results, fields) {
                                    if (error) {
                                        console.log(error);
                                    } else {
                                        if (results.length > 0) {
